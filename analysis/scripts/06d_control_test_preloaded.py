"""
06d_control_test_preloaded.py

Pre-loaded vectorized version. Same algorithm as 06b/06c but eliminates
the NetCDF-read-per-timestep bottleneck by loading all 80 timesteps of
u, v fields into numpy arrays once per season, then doing pure-numpy
indexing inside the advection loop.

Expected runtime: ~3-5 minutes for the full 28-run matrix.

Output: results/control_comparison_preloaded.csv
"""
import numpy as np
import xarray as xr
import pandas as pd
...
def run_ensemble_preloaded(u_stack, v_stack, grid_lat, grid_lon, lat_desc, lon_wrap,
                             release_lats, release_lons):
    """Run an ensemble using pre-loaded u_stack, v_stack of shape (N_STEPS+1, nlat, nlon)."""
    ...
    for s in range(N_STEPS):
        ...
        if active.sum() > 0:
            query_lons = lons[active].copy()
            if lon_wrap:
                query_lons = np.where(query_lons < 0, query_lons + 360, query_lons)
            pts = np.column_stack([lats[active], query_lons])
            u_at = fu(pts)
            v_at = fv(pts)
        ...
        if newly_out.any():
            for i in np.where(newly_out)[0]:
                traj_lats[i, s + 1:] = lats[i]
                traj_lons[i, s + 1:] = lons[i]
...
def run_year(year, ds, ceres, olr):
    ...
    if lat_desc:
        grid_lat_up = grid_lat[::-1]
    else:
        grid_lat_up = grid_lat
    lon_wrap = grid_lon.min() >= 0
    ...
    for month in [1, 4, 7, 10]:
        ...
        # Pre-load u,v stack for this release window
        t0 = time.time()
        u_stack = ds["u"].isel(valid_time=slice(start_idx, start_idx + N_STEPS),
                                pressure_level=0).values
        v_stack = ds["v"].isel(valid_time=slice(start_idx, start_idx + N_STEPS),
                                pressure_level=0).values
        if lat_desc:
            u_stack = u_stack[:, ::-1, :]
            v_stack = v_stack[:, ::-1, :]
        ...
        t0 = time.time()
        amz_lats, amz_lons = run_ensemble_preloaded(
            u_stack, v_stack, grid_lat_up, grid_lon, lat_desc, lon_wrap,
            amazon_lats, amazon_lons,
        )
        atl_lats, atl_lons = run_ensemble_preloaded(
            u_stack, v_stack, grid_lat_up, grid_lon, lat_desc, lon_wrap,
            atlantic_lats, atlantic_lons,
        )
        t_advect = time.time() - t0
        ...
        print(
            ...
            f"n_amz={len(amz_olr)}  n_atl={len(atl_olr)}  load={t_load:.1f}s  advect={t_advect:.1f}s",
            flush=True,
        )

        results.append({
            "year": year, "month": month, "enso_phase": ENSO_PHASE[year],
...
def main():
    print("=" * 70, flush=True)
    print("Extended Atlantic-source control (PRELOADED VECTORIZED, n=28)", flush=True)
    print("=" * 70, flush=True)
    ...
    out_csv = os.path.join(OUT_DIR, "control_comparison_preloaded.csv")
    df.to_csv(out_csv, index=False)

    print("\n" + "=" * 70, flush=True)
    print("EXTENDED CONTROL SUMMARY — PRELOADED (n=28)", flush=True)
    print("=" * 70, flush=True)
